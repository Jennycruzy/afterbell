...
AFTERBELL is a read-only safety layer for tokenized U.S. equities. It watches the
...
It is read-only. Nothing typed into the page can place an order.
...
## The two statuses that were easy to confuse

The project has two separate questions, and the dashboard now presents them
separately.

**Is there enough data to compare today with normal conditions?** Yes. Every
supported token has the required regular-hours sample count, and the holiday
period is represented in the report.

**Are the proposed safety limits approved for live use?** Not yet. The signed
settings file still contains draft limits copied from the initial design. A
project owner must compare those limits with the measured distributions and
explicitly approve or revise them. Until that decision is recorded, AFTERBELL
keeps live-order permission paused.

That is not a missing-data error and it is not a request for someone to click a
button on the website. It is an intentional safety hold: complete measurements
do not automatically authorize the limits derived from them.
...
- **Fail-closed:** missing reference data, missing liquidity data, an unknown
  instrument, an unsafe status, or missing account evidence cannot silently
  become a pass.
- **Tamper-evident:** every decision and refusal enters an append-only,
  hash-linked receipt ledger.
- **Credential-separated:** the recorder, dashboard, and evaluator hold no
  exchange credential. A supported client is the only place an approved,
  explicitly authorized submission could occur.
...
recorded token books + reference prices + exchange calendar
             ...
             smallest safe amount + plain-language explanation
                              │
                              ▼
                 signed authorization boundary (still paused)
                              │
                              ▼
                   hash-linked receipt ledger
```
...
public MCP surface exposes read-only evaluation and market-state tools. The
executor is disabled in the shipped settings and the public dashboard cannot
change that.
...
The read-only MCP endpoint provides `evaluate_order` and `get_market_state`.
The public dashboard and MCP server never place an order.
...
- [`docs/live-acceptance.md`](docs/live-acceptance.md) — the one recorded,
  manually approved minimum-size acceptance event.
